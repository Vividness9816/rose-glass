...
- **Phase 13 (semantic search — post-v1.0 add)** — `/council` → **ADR-20260618**: **brute-force cosine KNN in pure Rust** over the existing `embeddings` BLOBs, **NOT sqlite-vec** (the vectors already live in-Rust; a linear scan is sub-perceptible for a personal vault; sqlite-vec/HNSW = the named upgrade past ~100k notes). Pure `knn.rs` core (true cosine, self-exclude, NaN/zero/dim/k guards) + `read_embeddings`/`embedding_freshness` (model-filtered) + two IPC commands: `related_notes` (model-free — the note's vector is already stored) + `semantic_search` (per-call ONNX, lock-split). A "Related" list in the editor surfaces it (self-heals on recompute; surfaces the freshness "recompute to enable" empty-state — the council's load-bearing find: embeddings are only written by the manual recompute, never the indexer). 3-lens reviewed (no HIGH; fixed model-filtered freshness, k/query IPC bounds, the empty-state UI). cargo 71 (+9 KNN/semantic) + **real-model `#[ignore]d` proof** (cooking query ranks the cooking note first). MCP tool + AppState model-cache deferred (decisions.md #8).
