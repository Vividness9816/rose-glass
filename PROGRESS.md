...
- **Phase 9 (foundation)** — `editorKind(path)` router (pure, vitest 4) routes markdown/txt→CM6, pdf/docx→a typed placeholder (no binary fed to CM6); **`.md/.txt/.pdf` file associations** declared in `tauri.conf.json` (A8's installer-registration half). Spike confirmed pdfjs-dist + mammoth install cleanly (removed again — wired in the engine increment, not left as dep debt). The heavy editor ENGINES (PDF.js/MuPDF true-edit + TipTap/docx) + a binary-read IPC + indexer-indexes-binaries are a **focused increment needing the app window** (your eyeball on the live editors); not faked tonight.
- **Phase 4 (interaction + WebGPU)** — graph gains wheel-zoom-to-cursor / drag-pan / drag-node / click-to-open, built renderer-agnostic on shared pure `camera`/`hitTest`/seedable `stepSimulation` (vitest 9 + GraphRenderer node-lookup). **WebGPU primary renderer BUILT** (`WebGpuGraphRenderer`, instanced discs + edge lines, `@webgpu/types`-validated) behind a `probeWebGpu()` + GPU/2D toggle with an **airtight fallback** (create()→null on any failure + per-backend canvas `key` remount + device-loss recovery → canvas-2D; a GPU failure can never blank the graph). 3-lens review: 10 findings, all fixed (incl. the HIGH fallback-blank bug). **WebGPU pixels are BUILT, NOT GPU-VERIFIED — RTX 5090 eyeball pending** (no false "proven"; the predecessor's exact trap). d3-force decision recorded (decisions.md #9/#10).
...
- **Phase 4 — DONE pending eyeball**: interaction (zoom/pan/drag/click-open) + the airtight WebGPU↔2D fallback are built + 3-lens reviewed + tested. The **WebGPU render itself is BUILT but NOT GPU-VERIFIED** → run the app, click the **GPU** toggle in the graph header on the RTX 5090, confirm it renders at 4K + the interaction feels right (the predecessor's "verified-but-not" trap — do not believe "proven" without your eyes). GPU edges are leaner than 2D by design (renderer header documents it); decide parity after.
...
- **Phase 9 — FOUNDATION done** (editorKind router + `.md/.txt/.pdf` file associations); the heavy editor **engines** (PDF.js/MuPDF true-edit + TipTap/docx + binary IO + indexer-indexes-binaries) are the next **focused increment with you at the app window** to verify the live editors — not faked tonight.
- **Phases 6 + 7 + 10 + 11 — DONE** (backdrop/glass, terminal, MCP sidecar, neural clusters). Remaining to v1.0: the engine increment (Phase 9), the eyeballs above (8/4/6), **Phase 12 — full §20 v1.0 acceptance gate**.
