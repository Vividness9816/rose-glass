...
|---|---|---|---|
| A1 | Boots clean, no runtime errors, `tsc` passes, unit tests green, Ponytail audit clean | 🟡 | `tsc` 0; `vitest` 15/15; `vite build` 0; `cargo test` 23/23 + `cargo clippy` 0 warnings (`phase3-editor`). Remaining: live `tauri dev` window eyeball; formal `/ponytail-audit`. |
...
- Editor host: open-first-note on vault open; wikilink-click + backlink-click navigation; debounced autosave with an anti-clobber guard (never reload the buffer the user is editing; ignore our own save echo) — 8 vitest unit tests.
