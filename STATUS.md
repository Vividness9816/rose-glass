...
| A9 | External MCP client can `search` + `get_semantic_clusters` | ✅ | `rose-glass-mcp` bin — a read-only MCP server (JSON-RPC 2.0, MCP 2025-06-18) over stdio. Re-runnable: `cargo test` (9 mcp dispatch tests) + the e2e stdio transcript `docs/proof/phase10-mcp-session.txt`. An external client configures it (`claude mcp add rose-glass -- rose-glass-mcp --vault <dir>`). `search` is live; `get_semantic_clusters` returns data once Phase 11 populates `clusters` (`phase10-mcp`). |
