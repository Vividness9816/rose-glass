...
|---|---|---|---|
| A1 | Boots clean, no runtime errors, `tsc` passes, unit tests green, Ponytail audit clean | 🟡 | `tsc` 0; `vitest` **27/27**; `vite build` 0 (three.js lazy-split); `cargo test` **41/41** + `cargo clippy` 0 (+ Phase 11 embeddings/k-means; 2 `#[ignore]`d ONNX tests pass on demand); web render verified live via Playwright, 0 console errors. Remaining: live `tauri dev` window eyeball; formal `/ponytail-audit`. |
