/* DotField — reactbits.dev dot-field background, ported to strict TS for the graph pane.
   A cursor-reactive dot grid (bulge + radial glow) drawn on its own canvas-2D layer that
   sits BEHIND the transparent graph canvas (graph.css). Colors are passed in (resolved from
   tokens by the caller — canvas can't read CSS vars). Mount only when motion is allowed; the
   caller gates on prefers-reduced-motion so there's no RAF for users who opt out. */
...
export const DotField = memo(function DotField({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- one-shot setup; live props read via propsRef
  }, []);
